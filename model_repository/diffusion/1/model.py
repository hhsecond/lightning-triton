...
class StableDiffusionModel(LightningModule):
    def __init__(self, model_root: Path, device: torch.device):
        super().__init__()

        if not (model_root / "stable_diffusion_weights").exists():
            request.urlretrieve(
                "https://pl-public-data.s3.amazonaws.com/dream_stable_diffusion/sd_weights.tar.gz",
                "sd_weights.tar.gz"
            )
            file = tarfile.open("sd_weights.tar.gz")
            file.extractall(str(model_root/"stable_diffusion_weights"))

        config_path = model_root / "stable_diffusion_weights/sd_weights/v1-inference.yml"
        weights_path = model_root / "stable_diffusion_weights/sd_weights/sd-v1-4.ckpt"
        config = OmegaConf.load(f"{config_path}")
        config.model.params.cond_stage_config["params"] = {"device": device.type}
        self.model = load_model_from_config(config, f"{weights_path}")
        self.sampler = DDIMSampler(self.model)

    @typing.no_type_check
    def predict_step(
            self, prompts: List[str], height: int, width: int, num_inference_steps: int
    ) -> Any:
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        self.model = StableDiffusionModel(model_root=Path(__file__).parent, device=device)
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            # TODO - batch it before sending for prediction
            outputs = self.model.predict_step([input_text], height=512, width=512, num_inference_steps=4)
            decoded_image = outputs[0]
